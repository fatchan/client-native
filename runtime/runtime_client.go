...
func (c *client) AddSetCommitSSLCert(filename, content string) error {
	...
	resp, err := c.runtime.ExecuteRaw(fmt.Sprintf("new ssl cert %s", filename))
	fmt.Println(resp)
	if err != nil {
		fmt.Println(fmt.Errorf("%w", err))
		c.runtime.ExecuteRaw(fmt.Sprintf("abort ssl cert %s", filename))
		return nil
	}
	resp, err = c.runtime.ExecuteRaw(fmt.Sprintf("set ssl cert %s <<\n%s\n", filename, content))
	fmt.Println(resp)
	if err != nil {
		fmt.Println(fmt.Errorf("%w", err))
		c.runtime.ExecuteRaw(fmt.Sprintf("abort ssl cert %s", filename))
	...
	resp, err = c.runtime.ExecuteRaw(fmt.Sprintf("commit ssl cert %s", filename))
	fmt.Println(resp)
	if err != nil {
		fmt.Println(fmt.Errorf("%w", err))
		c.runtime.ExecuteRaw(fmt.Sprintf("abort ssl cert %s", filename))
	...
	resp, err = c.runtime.ExecuteRaw(fmt.Sprintf("add ssl crt-list /etc/haproxy/certs %s", filename))
	fmt.Println(resp)
	if err != nil {
		fmt.Println(fmt.Errorf("%w", err))
		return nil
...
func (c *client) DelSSLCert(filename string) error {
	resp, err := c.runtime.ExecuteRaw(fmt.Sprintf("del ssl crt-list /etc/haproxy/certs %s", filename))
	fmt.Println(resp)
	if err != nil {
		fmt.Println(fmt.Errorf("%w", err))
		c.runtime.ExecuteRaw(fmt.Sprintf("abort ssl cert %s", filename))
	...
	resp, err = c.runtime.ExecuteRaw(fmt.Sprintf("del ssl cert %s", filename))
	fmt.Println(resp)
	if err != nil {
		fmt.Println(fmt.Errorf("%w", err))
		return nil
...
func (c *client) OverwriteMapPayloadVersioned(name string, entries models.MapEntries) error {
	if c.runtime == nil {
		return fmt.Errorf("no valid runtime found")
	}
	name, err := c.GetMapsPath(name)
	if err != nil {
		return err
	...
	if err != nil {
		...
		fmt.Printf("client-native:OverwriteMapPayloadVersioned err: %v", err)
		return err

	}
	// fmt.Printf("client-native:OverwriteMapPayloadVersioned AddMapPayloadVersioned: %v %v %v", version, name, payload[i]);
	err = c.runtime.ClearMapVersioned(name, version)
	if err != nil {
