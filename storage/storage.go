...
type Storage interface {
	...
	Create(name string, contents io.ReadCloser) (string, int64, string, error)
}
...
func (s *storage) Create(name string, readCloser io.ReadCloser) (string, int64, string, error) {
	...
	switch s.fileType { //nolint:exhaustive
	case SSLType:
		_, _, b, err = s.createSSL(f, readCloser)
	case MapsType, GeneralType:
		_, _, b, err = s.createFile(f, readCloser)
	}
	if err != nil {
		return "", -1, "", conf.NewConfError(conf.ErrObjectAlreadyExists, fmt.Sprintf("Error creating file %s", f))
	}
	return f, -1, b, nil
}

func (s *storage) createSSL(name string, readCloser io.ReadCloser) (string, int64, string, error) {
	...
	return name, int64(len(b)), fmt.Sprintf("%s\n", b), nil
}

func (s *storage) createFile(name string, readCloser io.ReadCloser) (string, int64, string, error) {
	b, err := io.ReadAll(readCloser)
	if err != nil {
		return "", -1, "", err
	}
